...
try:
    ...
    HAS_NNCF = False
    print("Warning: nncf not installed. Quantization will be disabled.")
...
class NNCFQuantizer:
    """
    NNCF Quantization Manager for INT8 optimization
    
    Based on OpenVINO's NNCF quantization-aware training workflow.
    """
    ...
    def __init__(self, config: TrainingConfig):
        ...
        if not HAS_NNCF:
            print("✗ NNCF not available. Quantization disabled.")
            return
        
        # Set NNCF log level
        try:
            import logging
            nncf.set_log_level(logging.WARNING)
        except Exception as e:
            raise e
                    
        print("✓ NNCF initialized for quantization")
    
    def create_calibration_dataset(
        self, 
        dataloader: DataLoader, 
        num_samples: int = 300
    ) -> Optional['nncf.Dataset']:
        """
        Create calibration dataset for NNCF quantization
        ...
        Returns:
            nncf.Dataset for quantization
        """
        if not HAS_NNCF:
            return None
        
        # Transform function to extract input tensor from (image, target) tuple
        def transform_fn(data_item):
            image, _ = data_item
            return image
        
        # Create a subset dataloader with batch_size=1 for calibration
        calibration_data = []
        count = 0
        for batch in dataloader:
            ...
            for i in range(images.size(0)):
                if count >= num_samples:
                    break
                calibration_data.append((images[i:i+1], targets[i] if isinstance(targets, list) else None))
                count += 1
            if count >= num_samples:
                break
        ...
        print(f"✓ Created calibration dataset with {len(calibration_data)} samples")
    ...
    def quantize_model(
        ...
        Args:
            ...
            subset_size: Number of samples for quantization
        
        Returns:
            Quantized INT8 model
        """
        if not HAS_NNCF:
            print("✗ NNCF not available. Returning original model.")
            return model
        
        print("\n" + "=" * 60)
        print("Starting NNCF Quantization (FP32 → INT8)")
        print("=" * 60)
        ...
        quantized_model = nncf.quantize(
            ...
            preset=nncf.QuantizationPreset.MIXED,  # Balance accuracy and performance
    ...
    def fine_tune_quantized_model(
        ...
        """
        print("\n" + "=" * 60)
        print(f"Quantization-Aware Training ({num_epochs} epochs)")
        print("=" * 60)
        
        device = config.device
        quantized_model = quantized_model.to(device)
        
        # Use reduced learning rate for QAT (typically 1/10 of original)
        qat_lr = config.qat_learning_rate
        optimizer = optim.Adam(quantized_model.parameters(), lr=qat_lr)
        ...
        for epoch in range(num_epochs):
            ...
            for batch_idx, (images, targets) in enumerate(train_loader):
                ...
                # Forward pass
                pred_cls, pred_boxes = quantized_model(images)
                loss, loss_dict = criterion(pred_cls, pred_boxes, targets, anchors)
                
                # Backward pass
                optimizer.zero_grad()
                loss.backward()
                torch.nn.utils.clip_grad_norm_(quantized_model.parameters(), config.gradient_clip)
                optimizer.step()
                ...
                if batch_idx % 50 == 0:
                    print(f"  QAT Epoch {epoch+1}/{num_epochs}, Batch {batch_idx}/{len(train_loader)}, "
                          f"Loss: {loss.item():.4f}")
            
            avg_loss = epoch_loss / num_batches
            print(f"  QAT Epoch {epoch+1}/{num_epochs} complete. Avg Loss: {avg_loss:.4f}")
...
def main():
    ...
    quantizer = NNCFQuantizer(config) if config.enable_quantization else None
    ...
    fp32_ir_path = exporter.export_to_openvino(
        model,
    ...
    # ========================================================================
    # PHASE 3: NNCF Quantization
    # ========================================================================
    if config.enable_quantization and quantizer and HAS_NNCF:
        print("\n" + "=" * 60)
        print("PHASE 3: NNCF INT8 Quantization")
        print("=" * 60)
        
        # Need criterion for QAT
        criterion = SSDLoss(
            num_classes=config.num_classes,
            neg_pos_ratio=config.neg_pos_ratio,
            use_focal_loss=config.use_focal_loss,
        ...
        calibration_dataset = quantizer.create_calibration_dataset(
            val_loader,
        ...
        quantized_model = quantizer.quantize_model(
            model,
            calibration_dataset,
            subset_size=config.calibration_samples
        )
        
        # ====================================================================
        # PHASE 4: Quantization-Aware Training (QAT)
        # ====================================================================
        quantized_model = quantizer.fine_tune_quantized_model(
            quantized_model,
            train_loader,
            criterion,
            anchors,
        ...
        if config.use_azurite:
            azurite_client.put_object(
                config.azurite_model_bucket,
                'person_detector_ssd/best_model_int8.pth',
                int8_checkpoint_path
            )
        
        # ====================================================================
        # PHASE 5: Export INT8 to OpenVINO
        # ====================================================================
        print("\n" + "=" * 60)
        print("PHASE 5: Export INT8 Model to OpenVINO")
        print("=" * 60)
        
        int8_ir_path = exporter.export_to_openvino(
            quantized_model,
        ...
        # ====================================================================
        if fp32_ir_path and int8_ir_path:
            comparison_results = exporter.compare_models(
                fp32_path=fp32_ir_path,
                int8_path=int8_ir_path,
                ...
                iou_thresholds=[0.5, 0.75]  # mAP@0.50 and mAP@0.75
            )
            
            # Upload OpenVINO models to Azurite
            if config.use_azurite:
                for model_file in [fp32_ir_path, fp32_ir_path.replace('.xml', '.bin'),
                                  int8_ir_path, int8_ir_path.replace('.xml', '.bin')]:
                    if os.path.exists(model_file):
                        azurite_client.put_object(
                            config.azurite_model_bucket,
                            f'person_detector_ssd/{os.path.basename(model_file)}',
                            model_file
                        )
    ...
    if config.enable_quantization:
        print("  - best_model_int8.pth (PyTorch INT8)")
        print("  - person_detector_int8.xml/bin (OpenVINO INT8)")
